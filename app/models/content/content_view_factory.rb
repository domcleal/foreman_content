module Content
  class ContentViewFactory
    ...
    attr_accessor :type, :originator_id, :parent_cv, :product_cvs
    # create a content view of a single product
    def self.create_product_content_view(product_id)
      product = Product.find_by_id(product_id)
      ContentView.new(:source_repositories => product.repositories, :originator_name => product.name,
                      :originator => product)
    end
    ...
    def self.create_os_content_view(operatingsystem_id)
      ...
      ContentView.new(:source_repositories => os.repositories, :originator_name => os.to_label,
                      :originator => os)
    end
    ...
    def self.create_composite_content_view(options = {})
      ...
      clones = Content::RepositoryClone.joins(:content_view_repository_clones).
        where(:content_content_view_repository_clones => {:content_view_id => product.cvs})
      ContentView.new(:source_repositories=> [], :originator_name => hostgroup.name,
                      :originator => hostgroup, :repository_clones => clones, :parent_id => factory.parent_cv)
    end
